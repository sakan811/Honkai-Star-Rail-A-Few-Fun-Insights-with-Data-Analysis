...
def test_create_views(mock_sqlite_connection):
    ...
    with patch('hsrws.db.sqlite.get_element_char_count_by_ver', return_value="CREATE VIEW ElementCharacterCountByVersion AS SELECT..."), \
         patch('hsrws.db.sqlite.get_path_char_count_by_ver', return_value="CREATE VIEW PathCharacterCountByVersion AS SELECT..."), \
         patch('hsrws.db.sqlite.get_rarity_char_count_by_ver', return_value="CREATE VIEW RarityCharacterCountByVersion AS SELECT..."):
    ...
    with patch("sqlite3.connect") as mock_connect:
        ...
        with patch("hsrws.db.sqlite.drop_views") as mock_drop_views:
            with patch("hsrws.db.sqlite.create_views") as mock_create_views:
                ...
                sample_character_df.to_sql.assert_called_once_with("HsrCharacters", mock_conn, if_exists="replace")
