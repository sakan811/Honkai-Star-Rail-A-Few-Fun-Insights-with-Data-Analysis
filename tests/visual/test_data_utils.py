...
def test_get_latest_patch():
    ...
    with patch("hsrws.visual.data_utils.get_latest_patch_stmt") as mock_stmt, \
...
def test_get_element_path_heatmap_data(mock_session, mock_execute_result):
    ...
    with patch("hsrws.visual.data_utils.get_element_path_heatmap_stmt") as mock_stmt:
        with patch("hsrws.visual.data_utils.fetch_data_orm") as mock_fetch:
            mock_fetch.return_value = pd.DataFrame({"Element": ["Fire"], "Path": ["Hunt"], "Count": [2]})
...
def test_get_rarity_element_distribution_data(mock_session, mock_execute_result):
    ...
    # Execute
    with patch("hsrws.visual.data_utils.get_rarity_element_distribution_stmt") as mock_stmt:
        with patch("hsrws.visual.data_utils.fetch_data_orm") as mock_fetch:
            mock_fetch.return_value = pd.DataFrame({"Rarity": [5], "Element": ["Fire"], "Count": [2]})
...
def test_get_version_release_timeline_data(mock_session, mock_execute_result):
    ...
    # Execute
    with patch("hsrws.visual.data_utils.get_version_release_timeline_stmt") as mock_stmt:
        with patch("hsrws.visual.data_utils.fetch_data_orm") as mock_fetch:
            mock_fetch.return_value = pd.DataFrame({"Version": ["1.0"], "New_Characters": [5]})
...
def test_get_element_balance_evolution_data(mock_session, mock_execute_result):
    ...
    with patch("hsrws.visual.data_utils.fetch_view_data") as mock_fetch_view:
        mock_fetch_view.return_value = pd.DataFrame({"Version": ["1.0"], "Fire": [1], "Ice": [1]})
...
def test_get_element_balance_evolution_data_fallback(mock_session, mock_execute_result):
    ...
    with patch("hsrws.visual.data_utils.fetch_view_data") as mock_fetch_view:
        ...
        with patch("hsrws.visual.data_utils.get_version_element_evolution_stmt") as mock_stmt:
            with patch("hsrws.visual.data_utils.fetch_data_orm") as mock_fetch:
                mock_fetch.return_value = pd.DataFrame({"Version": ["1.0"], "Fire": [1], "Ice": [1]})
                result = get_element_balance_evolution_data()
...
def test_get_path_rarity_distribution_data(mock_session, mock_execute_result):
    ...
    # Execute
    with patch("hsrws.visual.data_utils.get_path_rarity_distribution_stmt") as mock_stmt:
        with patch("hsrws.visual.data_utils.fetch_data_orm") as mock_fetch:
            mock_fetch.return_value = pd.DataFrame({"Path": ["Hunt"], "5-star": [2], "4-star": [1]})
