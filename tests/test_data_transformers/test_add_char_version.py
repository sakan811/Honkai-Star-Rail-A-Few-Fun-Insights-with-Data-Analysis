...
def test_add_char_version(sample_dataframe):
    ...
    mock_get_version_dict = {2.0: ["Spider-Man", "Iron Man"]}
    ...
    sample_dataframe["Version"] = sample_dataframe["Character"].apply(
        lambda character: next(
            (
                version
                for version, characters in mock_get_version_dict.items()
    ...
    assert sample_dataframe["Version"].equals(
        pd.Series([1.0, 1.0, 2.0, 2.0], name="Version")
    )
