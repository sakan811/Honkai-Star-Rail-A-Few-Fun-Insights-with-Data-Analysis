...
def _setup_chart_basics(
    fig, ax, config, chart_type, title, patch_version=None, x_label=None, y_label=None
):
    """
    ...
    if x_label:
        ax.set_xlabel(
            x_label, fontsize=config.get_font_size("label", chart_type), labelpad=10
        )
    if y_label:
        ax.set_ylabel(
            y_label, fontsize=config.get_font_size("label", chart_type), labelpad=10
        )

    # Set tick label font sizes
    ax.tick_params(
        axis="both", which="major", labelsize=config.get_font_size("tick", chart_type)
    )
